...
**Thinking Chronology Phase:** Phase 4 (Clarity — what gets hosted IS the belief), Phase 6 (Execution — reliable delivery at scale).

**Vault connections:**
- `[[BELIEF + SPEC.]]` — What gets deployed is the belief the user buys. Every pixel served through hosting carries Jobs' reality distortion. The hosting architecture itself must be seamless because the product IS the experience.
- `[[Murthy (Infosys)]]` — Execution excellence in deployment. Reliable, repeatable, zero-downtime deploys. The CDN must serve with Infosys-grade reliability.
- `[[OPERATIONS/TOOLS AND VENDORS/NRI Payment]]` — Hosting must serve NRI users globally with low latency. CDN edge placement matters.
- `[[Bezos (Amazon)]]` — Operational infrastructure that survives 10x traffic without breaking. Day 1 deployment discipline.

**Customization:** "Run hosting without Murthy mindset."
