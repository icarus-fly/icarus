...
**Thinking Chronology Phase:** Phase 6 (Architecture — data layer), Phase 8 (Scale).

**Vault connections:**
- `[[TRAQA]]` — Core data storage for all user health reports, family trees, and longitudinal health data. Every report upload, every family connection, every health trend — stored here.
- `[[TECHNOLOGY BRAIN]]` — Data architecture decisions. How data is structured determines how fast and flexible the product is.
- `[[SCALABILITY PLAN]]` — Firestore must scale from 100 users to 10 million without re-architecture. Index design today determines query performance at scale.
- `[[Bezos (Amazon)]]` — Customer-obsessed data access. Users should see their data instantly. Every millisecond of latency is customer friction.

**Customization:** "Run firestore-standard without Scale mindset."
