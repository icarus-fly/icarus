...
**Thinking Chronology Phase:** Phase 5 (Defense — competitive security), Phase 6 (Trust — customer data protection).

**Vault connections:**
- `[[NON- NEGOTIABLES]]` — Healthcare data is regulated. Security rules are not optional — they are legal requirements under IPC and DPDP. Every rule must pass the compliance audit.
- `[[Sun Tzu]]` — Security as competitive defense. Know yourself (your data structure), know your enemy (attack vectors), and build rules that make the cost of attack higher than the value of data.
- `[[Bezos (Amazon)]]` — Customer trust is the most valuable asset. One data breach destroys years of trust. Security rules are the first line of defense protecting customer obsession.
- `[[Knight (Nike Mindset)]]` — Users trust Traqa with their most sensitive health data. That trust IS the identity they buy into. Security breaches destroy the identity they became.

**Customization:** "Run security-rules without Sun Tzu."
